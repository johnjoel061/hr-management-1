...
const useUpdateDesignationById = () => {
  const [loading, setLoading] = useState(false);

  const updateDesignationById = async (id, updatedData) => {
    setLoading(true);
    try {
      ...
      Object.keys(updatedData).forEach((key) => {
        if (key === 'desScannedPicture' && Array.isArray(updatedData[key])) {
          updatedData[key].forEach((item) => {
            if (typeof item === 'string') {
              // Append existing image paths as strings
              formData.append('desScannedPicture[]', item);
            } else {
  ...
  return { updateDesignationById, loading };
};

export default useUpdateDesignationById;
